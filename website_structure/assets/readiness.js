(function(){
  ...
  var shared = [
    ...
    {id:"q7", dimension:"adoption", title:"Could the team run a new protocol without the designer in the room?", options:["No; the work depends on a few individuals.","Only with frequent support and interpretation.","Most operators could follow a documented protocol.","Operators can run, challenge and improve the protocol themselves."]},
    ...
    {id:"q11", dimension:"adoption", title:"Who would own adoption of the first workflow?", options:["No one is clearly accountable yet.","A likely sponsor exists, but ownership is informal.","A sponsor and operational owner can be named.","A sponsor, owner and operator group are ready to support a bounded pilot."]},
    {id:"q12", dimension:"learning", title:"How would you know the first workflow had improved?", options:["We have not agreed what to measure.","We would rely mainly on anecdotal feedback.","A few practical measures could be defined before a pilot.","Measures, baseline evidence and a review point are already understood."]}
  ];
  ...
  var routeQuestions = {
    bid: [
      ...
      {id:"q10", title:"Which workflow would you most like to test first?", nonScored:true, options:["Compliance and response matrix","Go / No-Go and strategy gate","Evidence and citation control","Red-team and submission assurance","I am not sure yet"]}
    ],
    tender: [
      ...
      {id:"q10", title:"Which workflow would you most like to test first?", nonScored:true, options:["Comparable evaluation matrix","Mandatory requirements checks","Clarification control","Evaluation assurance and moderation","I am not sure yet"]}
    ]
  };
  ...
  function submitAssessment(event){
    ...
    var payload={firstname:form.firstname.value.trim(),email:form.email.value.trim(),company_website:form.company_website.value,route:state.route,readiness_band:result.band,weakest_dimension:result.weakest,recommended_workflow:rec[0],constraint_answer:state.answers.q9.label,workflow_interest:state.answers.q10.label,source:"commercial_ai_readiness_check",page_url:window.location.href,answers:{}};
